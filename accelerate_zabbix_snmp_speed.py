...
for item in items:
    if "Operational status" in item.get("name", ""):
        ...
        print(f"Accelerated Zabbix SNMP item '{item['name']}' polling interval to 3s!")
        updated_count += 1

print(f"Total {updated_count} SNMP items accelerated to 3s real-time refresh speed!")
